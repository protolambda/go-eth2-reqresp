...
type ResponseChunkHandler func(ctx context.Context, chunkIndex uint64, chunkSize uint64, result ResponseCode, r io.Reader) error
...
func (handleChunk ResponseChunkHandler) MakeResponseHandler(maxChunkCount uint64, maxChunkContentSize uint64, comp Compression) ResponseHandler {
	...
	return func(ctx context.Context, r io.ReadCloser) error {
		...
		for chunkIndex := uint64(0); chunkIndex < maxChunkCount; chunkIndex++ {
			...
			if err := handleChunk(ctx, chunkIndex, chunkSize, ResponseCode(resByte), cr); err != nil {
				return err
			}
		}
		return nil
